...
namespace tmp
{
    public class Render
    {
        ...
        public void Initialise(int width, int height)
        {
            ...
            Resize(width, height);
            skyBoxShaderProgram = Shaders.GetSkyBoxShader();
            texture = Texture.GetCubeMap(Directory.GetFiles(Path.Combine("data", "textures", "skybox"), "*.png")
                .ToList());
            arrayTex = Texture.InitArray(Directory.GetFiles(Path.Combine("data", "Textures"), "*.png").ToList());
            InitCubes();
            InitBuffers();
        }
